...
namespace SchoolLibrary_EF.API.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public UserController(IUserService userService, ILoggerFactory loggerFactory)
        {
            ...
            _logger = loggerFactory.CreateLogger($"{this.GetType().Name}Logger");
        }
        ...
        public async Task<ActionResult<IEnumerable<UserDTO>>> GetAllAsync()
        {
            ...
            {
                _logger.LogError
                    ("Error in [{typeName}]->[GetAllAsync] => {ex.Message}", this.GetType().Name, ex.Message);
        ...
        public async Task<ActionResult<UserDTO>> GetByIdAsync(Guid id)
        {
            try
            {
                ...
                if (entity == null)
                {
                    _logger.LogError("Entity with id: [{id}] from [Users] not found", id);
                ...
                else
                {
                    _logger.LogInformation
                        ("Entity with id: [{UserId}] were successfully extracted from [Users]", entity.UserId);
            ...
            {
                _logger.LogError
                    ("Error in [{typeName}]->[GetByIdAsync] => {ex.Message}", this.GetType().Name, ex.Message);
        ...
        public async Task<ActionResult> AddAsync(UserDTO newUser)
        {
            try
            {
                ...
                else
                {
                    ...
                    _logger.LogInformation
                        ("Entity with id: [{UserId}] were successfully added to [Users]", newUser.UserId);
            ...
            {
                _logger.LogError
                    ("Error in [{typeName}]->[AddAsync] => {ex.Message}", this.GetType().Name, ex.Message);
        ...
        public async Task<ActionResult> UpdateAsync(UserDTO updateUser)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (findResult == null)
                    {
                        _logger.LogError("Entity with id: [{id}] from [Users] not found", updateUser.UserId);
                    ...
                    else
                    {
                        ...
                        _logger.LogInformation
                            ("Entity with id: [{UserId}] were successfully updated from [Users]", updateUser.UserId);
            ...
            {
                _logger.LogError
                    ("Error in [{typeName}]->[UpdateAsync] => {ex.Message}", this.GetType().Name, ex.Message);

                return StatusCode(StatusCodes.Status500InternalServerError, ex.Message);
            }
        }

        [HttpDelete("{id}")] // DELETE: ef/user
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult> DeleteAsync(Guid id)
        {
            try
            {
                ...
                else
                {
                    ...
                    _logger.LogInformation
                        ("Entity with id: [{UserId}] were successfully deleted from [Users]", id);
            ...
            {
                _logger.LogError
                    ("Error in [{typeName}]->[DeleteAsync] => {ex.Message}", this.GetType().Name, ex.Message);
