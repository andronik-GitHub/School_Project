...
namespace SchoolLibrary_EF.DAL.Helper
{
    public class SortHelper<T> : ISortHelper<T>
    {
        public IQueryable<T> ApplySort(IQueryable<T> entities, string? orderByString)
        {
            ...
            var orderParam = orderByString.Trim().Split(',');
            ...
            foreach (var param in orderParam)
            {
                ...
                var propFromQueryName = param.Split(' ')[0]; // get parameter name
                ...
                strBuilder.Append($"{objProperty.Name.ToString()} {sortOrder}, ");
            }


            // Removing excess commas
            var orderQuery = orderByString.ToString().TrimEnd(',', ' ');
